...
PHASE_NAMES=('cond','evap','dep','sub','frz','mlt')

def psat_liquid(T):
    """Buck liquid-water saturation pressure, including supercooled liquid."""
    c=np.asarray(T)-TF
...
def psat(T):
    """Compatibility alias: membrane activity is relative to liquid water."""
...
class Model:
    def __init__(self, bp=False, scale=1, j0=.1, kf=1., km=1., kvl=1., kvi=1e-4,
                 lambda_nf=3., exchange_factor=1., kcond=None, kevap=None, kdep=None, ksub=None):
        ...
        self.kcond=kvl if kcond is None else kcond
        self.kevap=kvl if kevap is None else kevap
        self.kdep=kvi if kdep is None else kdep
        self.ksub=kvi if ksub is None else ksub
        if min(kf,km,self.kcond,self.kevap,self.kdep,self.ksub)<0:
    ...
    def mass_step(self,T,mv0,ml0,mi0,j,dt):
        ...
        # Actual paired transfers, not unconstrained source rates, enter budgets.
        mv,ml,mi,phase_heat,amounts=self.phase_step(temp,mv,ml,mi,dt)
        adsorption=float(np.dot(heat,self.dx))
        heat+=phase_heat
        amounts['adsorption_heat_J_m2']=adsorption
        return mv,ml,mi,heat,produced,out,amounts

    def phase_step(self,temp,mv0,ml0,mi0,dt):
        """First-order operator split: vapor/liquid, vapor/ice, liquid/ice.
        Pairwise BE relaxation with donor limits; exact local mass transfer.
        Coefficients use alpha/tau_star, tau_star=1 s (explicit closure).
        No induction/growth submodel or experimental V/T is used.
        """
        mv=mv0.copy();ml=ml0.copy();mi=mi0.copy();heat=np.zeros(self.n)
        amounts={name:np.zeros(self.n) for name in PHASE_NAMES}; ids=self.pore
        gas=np.maximum(self.fractions(ml,mi),0)
        satl=gas[ids]*MW*psat_liquid(temp[ids])/(R*temp[ids])
        excess=mv[ids]-satl
        cond=np.maximum(excess,0)*(dt*self.kcond)/(1+dt*self.kcond)
        evap=np.minimum(ml[ids],np.maximum(-excess,0)*(dt*self.kevap)/(1+dt*self.kevap))
        mv[ids]+=evap-cond;ml[ids]+=cond-evap
        amounts['cond'][ids]=cond;amounts['evap'][ids]=evap
        heat[ids]+=LC*(cond-evap)
        gas=np.maximum(self.fractions(ml,mi),0)
        sati=gas[ids]*MW*psat_ice(temp[ids])/(R*temp[ids])
        excess=mv[ids]-sati
        # At/above Tf, ice melting represents solid disappearance; gas/ice off.
        cold=temp[ids]<TF
        dep=np.maximum(excess,0)*(dt*self.kdep)/(1+dt*self.kdep)*cold
        sub=np.minimum(mi[ids],np.maximum(-excess,0)*(dt*self.ksub)/(1+dt*self.ksub))*cold
        mv[ids]+=sub-dep;mi[ids]+=dep-sub
        amounts['dep'][ids]=dep;amounts['sub'][ids]=sub
        heat[ids]+=(LC+LF)*(dep-sub)
        freezable=ml.copy()
        freezable[self.mem]=np.maximum(ml[self.mem]-WPL*self.lambda_nf,0)
        af=self.kf*np.maximum((TF-temp)/TF,0)
        am=self.km*np.maximum((temp-TF)/TF,0)
        freeze=freezable*dt*af/(1+dt*af)
        melt=mi*dt*am/(1+dt*am)
        ml+=melt-freeze;mi+=freeze-melt;heat+=LF*(freeze-melt)
        amounts['frz']=freeze;amounts['mlt']=melt
        totals={name:float(np.dot(value,self.dx)) for name,value in amounts.items()}
        totals['frz_pore']=float(np.dot(freeze[self.pore],self.dx[self.pore]))
        totals['frz_mem']=float(np.dot(freeze[self.mem],self.dx[self.mem]))
        totals['mlt_pore']=float(np.dot(melt[self.pore],self.dx[self.pore]))
        totals['mlt_mem']=float(np.dot(melt[self.mem],self.dx[self.mem]))
        return mv,ml,mi,heat,totals
    ...
    def run(self,t_samples,current,T0_C,dt=.025,fields=False):
        ...
        phase_sum={k:0. for k in (*PHASE_NAMES,'frz_pore','frz_mem','mlt_pore','mlt_mem')}
        phase_last=phase_sum.copy();last_sample=float(ts[0]);adsorption_sum=0.
        ...
        def record(tt,j):
            ...
            interval=tt-last_sample
            for name,value in phase_sum.items():
                row[f'phase_{name}_kg_m2']=value
                row[f'rate_{name}_kg_m2_s']=(value-phase_last[name])/interval if interval>0 else 0.
                phase_last[name]=value
            row['heat_adsorption_J_m2']=adsorption_sum
            phase_rebuilt=LC*(phase_sum['cond']-phase_sum['evap'])+LF*(phase_sum['frz']-phase_sum['mlt'])+(LC+LF)*(phase_sum['dep']-phase_sum['sub'])+adsorption_sum
            row['phase_heat_balance_J_m2']=qphase-phase_rebuilt
            row['ice_balance_kg_m2']=row['water_ice_kg_m2']-(phase_sum['frz']-phase_sum['mlt']+phase_sum['dep']-phase_sum['sub'])
            row['ice_to_produced_water_ratio']=row['water_ice_kg_m2']/sumprod if sumprod>0 else 0.
            last_sample=float(tt)
            rows.append(row)
            if fields:
        ...
        for target in ts[1:]:
            ...
            for _ in range(steps):
                ...
                mv,ml,mi,ph,prod,out,pc=self.mass_step(T,mv,ml,mi,j,h)
                ...
                for name in phase_sum:phase_sum[name]+=pc[name]
                adsorption_sum+=pc['adsorption_heat_J_m2']
