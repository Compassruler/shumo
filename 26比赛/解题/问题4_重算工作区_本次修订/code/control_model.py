...
@njit(cache=True)
def row(t,temp,e,d,q,en,wb,stopped,cs,water,estimated_temp,estimated_e,sensor):
    out=np.zeros(113)
...
def simulate_raw(cfg,observer_cfg,temp0,kind,p,power,dt,period,horizon,post,record,thermal,noise,sensor_shutdown):
    ...
    hist=np.empty((nmax,113));count=0
    if record:hist[count]=row(t,temp,e,d,q,en,wb,stopped,cs,water,estimated_temp,estimated_e,sensor);count+=1
    ...
    while t<(stop+post if stopped else startup_end)-1e-9:
        ...
        if record:
            estimated_e,_=evaluate(observer_cfg,estimated_states,estimated_temp,min(.005*t,.3),THERMAL)
            hist[count]=row(t,temp,e,d,q,en,wb,stopped,cs,water,estimated_temp,estimated_e,sensor);count+=1
