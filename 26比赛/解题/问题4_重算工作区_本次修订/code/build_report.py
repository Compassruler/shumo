...
STRATEGIES.update({'dynamic':'名义能耗优选动态','guarded':'推荐留裕度动态',
                  'optimized_constant':'同工况重优化恒功率','zero_heat':'零辅助加热'})
...
def build(root=ROOT):
    ...
    r.p('观测器使用独立副本状态，由电流输入、已施加功率及温度/电压采样推进；控制律不直接读取被控对象的真实冰、水、气体库存或真实端板温度。温度和电压经低通滤波并计算滤波速率，独立水相先验与无冰参考电压残差组合得到冰量估计。真实温度和真实冰量仅用于数值评估与估计误差检查，不作为隐藏的控制输入。观测器仍基于相同类型的物理模型，其模型误差覆盖范围以实际扰动试验为准。')
    ...
    r.p('真实首次成功t_first由五片真实均温均超过数值零阈值、当时状态和历史路径约束满足来评价。测量停机独立地检查每次采样的温度裕度、电压及观测冰量，连续满足2 s后才关热；使用0.3 K温度裕度，保持计时在条件失效时清零。由采样确定的停机时刻可以晚于真实首次成功2 s以上，这一差异通过表格和图16显示。零时刻已达标的首次事件直接记t_first=0。')
    ...
    r.formula('常规域：T_target∈[0.2,4] ℃；t_plan∈[5,190] s；K_P∈[0.025,0.65]\nK_I∈[0.0005,0.07]；taper_K∈[0.1,2] K；rate_gain∈[0,0.4]\n独立零名义功率分支：K_P=K_I=rate_gain=ff_factor=0，保留风险后备\n可行候选加权指标：J=E_aux/E_ref+0.005 t_stop/t_ref+0.002 ΔTmax/ΔT_ref\n推荐方案另要求在保存的扰动训练集内全部通过，再依能耗选取\n有限候选搜索不提供连续控制函数空间的全局极小证明')
    r.p('加权J以能耗为主，同时给予时间与一致性小权重；它不是严格字典序最小化。规划时间t_plan是参考轨迹参数，可以超过实际首次成功期限。恒功率固定C沿用问题三分配[1,1,0.6245115587719579,1,1] W/cm²；新增同工况恒功率再优化用于区分“优于固定旧分配”与“优于重新整定的恒功率”。')
    ...
    design=optional_table(r,root,'guarded_design_search.csv','推荐方案全部候选与训练筛选',
      ['case','candidate','T_target_C','t_plan_s','training_passed','training_count','all_training_passed','training_max_stop_s','feasible','E_aux_J','first_success_s','stop_s','min_voltage_V','max_ice_bulk'])
    ...
    optional_table(r,root,'zero_heat_results.csv','零辅助加热基线完整结果')
    ...
    r.table('两种能耗窗口、关热状态与累计加载量',allrows,
      ['case','strategy','first_success_s','first_success_energy_J','charge_at_success_C_cm2','stop_s','E_aux_J','final_min_T_C','final_max_T_C','final_left_EP_C','elapsed_s'],'guarded_results.csv + main_results.csv')
    ...
    r.table('全部策略启动能量与水量收支',allrows,
      ['case','strategy','E_aux_J','E_gen_J','E_phase_J','E_loss_J','E_sensible_J','energy_residual_J','water_residual_kg','max_water_residual_kg'],'guarded_results.csv + main_results.csv')
    r.table('全部策略物理可行性诊断',allrows,
      ['case','strategy','min_gas_porosity','max_j_over_jlim','min_kappa_S_m','min_inventory','max_iteration_error_K'],'guarded_results.csv + main_results.csv')
    image_if(r,root,'图09_能量收支与累积能耗','推荐动态与固定C的能量闭合及动态累计辅助电能')
    included=set()
    for fname,title in [('precooling_energy_balance.csv','全部预冷能量守恒'),('precooling_validation.csv','预冷验证'),('precooling_convergence.csv','预冷时间与空间收敛'),('startup_convergence.csv','固定控制参数的启动收敛'),('coupled_convergence.csv','启动空间与时间联合加密')]:
    ...
    r.p('温度、电压噪声进入真实采样链与保持判定；独立模型观测器根据自身状态和采样更新。初场、物理参数、观测器初始偏差以及随机种子按试验表实际值列示。高斯噪声无界，有限随机试验只能支持已测分布与幅值内的经验表现。即使全部通过，也不等于任意扰动下的鲁棒安全或无限时域稳定。')
